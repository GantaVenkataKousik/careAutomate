...
const ScheduleAppointment = () => {
  ...
  const handleCreateAppointment = async () => {
    // Validate startDate and timeSlot
    if (!startDate || !timeSlot) {
      console.error("Start date or time slot is missing.");
      toast.error("Please select a valid date and time slot.");
      return;
    }
  
    const startDateTime = new Date(`${startDate}T${timeSlot}`);
    ...
    const endDateTime = endDate && timeSlot ? new Date(`${endDate}T${timeSlot}`) : null;
    const formattedEndtime = endDateTime
      ? format(endDateTime, 'yyyy-MM-dd HH:mm')
      : calculateEndTime(startDateTime, timeDuration);

      console.log(formattedStarttime);
  
    const payload = {
      tenantId: title || 'Unknown',
      hcmId: hcmId || 'N/A',
      date: startDate,
  ...
  <input
    type="date"
    value={startDate || ''}
    onChange={(e) => setStartDate(e.target.value)} // Make sure this updates the state
