...
const PopupPage = () => {
  ...
  const [showScheduleConfirm, setShowScheduleConfirm] = useState(false); // For schedule confirmation
  ...
  const handleNext = () => {
    ...
    if (currentSubStep < currentStepSubSteps - 1) {
      ...
      // Check if next step is "Schedule"
      if (steps[currentStep + 1].name === "Schedule") {
        // Show confirmation popup for scheduling
  ...
  const handleScheduleConfirm = (proceed) => {
    if (proceed) {
      // Proceed to the "Schedule" step
      setCurrentStep((prev) => prev + 1);
      setCurrentSubStep(0);
    } else {
      // Skip the "Schedule" step and mark as complete
      setComplete(true);
    }

    setShowScheduleConfirm(false); // Close confirmation popup
  };
